...
std::vector<torch::Tensor> linear_cutlass_forward(
    ...
    CutlassSgemm(false, true, batch_size, out_features, in_features, alpha, input.data_ptr<float>(), lda, weights.data_ptr<float>(), ldb, beta, output.data_ptr<float>(), ldc, stream0);
